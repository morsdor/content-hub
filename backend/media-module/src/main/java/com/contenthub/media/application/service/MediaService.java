...
public class MediaService implements RequestPresignedUploadUseCase, ConfirmUploadUseCase {
	...
	@Override
	@Transactional
	public PresignedUpload requestUpload(UUID workspaceId, UUID cardId, String filename, String contentType,
			long sizeBytes) {
		String bucket = mediaStoragePort.resolveMediaBucket();
		String key = workspaceId + "/" + UUID.randomUUID() + "/" + filename;

		MediaAsset asset = MediaAsset.builder().workspaceId(workspaceId).cardId(cardId).s3Key(key).s3Bucket(bucket)
				.contentType(contentType).sizeBytes(sizeBytes).status(MediaStatus.UPLOADING).build();
	...
	public void confirmUpload(UUID mediaId, UUID callerUserId, String traceId) {
		...
		VideoUploadedEvent event = VideoUploadedEvent.builder().mediaId(mediaId).workspaceId(asset.getWorkspaceId())
				.cardId(asset.getCardId()).s3Key(asset.getS3Key()).s3Bucket(asset.getS3Bucket()).traceId(traceId)
				.build();

		try {
			String payload = objectMapper.writeValueAsString(event);
			outboxRepository.save(OutboxEntry.of("MediaAsset", mediaId, VideoUploadedEvent.EVENT_TYPE,
					VideoUploadedEvent.TOPIC, payload, traceId));
